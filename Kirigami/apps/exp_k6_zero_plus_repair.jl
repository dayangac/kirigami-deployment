...
#
#   julia --project=Kirigami Kirigami/apps/exp_k6_zero_plus_repair.jl [--n 200] [--maxf 800] [--out DIR]
...
# (data/corpus/k1a_200.json, verified bit-identical to results/experiments/k5/sigma) is used.
# Outputs go to results/experiments/k6/; the shape cache lives under <out>/cache.
...
function main(args::Vector{String})
    ...
    eps = 0.3
    outdir = joinpath(REPO, "results", "experiments", "k6")
    sigmadir = ""
    cache = ""
    ...
    do_native = true
    # K6 pass 3 weights, chosen on a 3-graph pilot (see EXPERIMENTS.md K6).
