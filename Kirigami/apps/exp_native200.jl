...
# of the 200 random graphs of the experiment population (make_graph(id, 100,
...
#   sigma_def: K5's defect-minimising sigma (results/experiments/k5/sigma/*.json), same
#              convention as K6.
#
...
# (no GNU coreutils `timeout` on this machine, same trick as exp_k6_zero_plus_repair.jl). Runs are
...
#
# K9's numbers for the SAME 200 graphs (already computed, results/experiments/k9/k9.csv:
...
#
#   julia --project=Kirigami Kirigami/apps/exp_native200.jl [--n 200] [--maxf 800]
...
# (baseline/README.md). Outputs go to results/experiments/native200/. `--limit K`
# stops after K cells.
...
# only where results/experiments/k5/sigma/<kind>_<id>.json is present and matches.
...
function main(args::Vector{String})
    ...
    # results/experiments/native200/cells_*.txt are derived from it.
