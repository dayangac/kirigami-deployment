...
#
# Background. K6 (results/experiments/EXPERIMENTS.md) repaired the 0+ jam inside the Tutte
...
# 11/12 zero-plus feasible and 4/12 fully certified (results/experiments/k7/k7_c3_all.csv).
...
#
#   julia --project=Kirigami Kirigami/apps/exp_b4_boundary_vs_graph.jl [--n 200] [--maxf 800] [--out DIR]
...
#         [--regenerate]
#
# Population as exp_k9_convex_embedding.jl (frozen k1a_200 + archived sigma_def). The shape caches:
# `--k6cache` for the fixed control, `--cache` for the two variants.
# Outputs go to results/experiments/b4/.
...
function main(args::Vector{String})
    ...
    eps = 0.3
    outdir = joinpath(REPO, "results", "experiments", "b4")
    sigmadir = ""
    cache = joinpath(REPO, "results", "experiments", "b4", "cache")
    k6cache = joinpath(REPO, "results", "experiments", "k6", "cache")
    shard = 0; nshards = 1
