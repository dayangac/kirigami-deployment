...
#
#   julia --project=Kirigami Kirigami/apps/exp_k5_orientation.jl [--n 200] [--maxf 800] [--cap 20]
...
# k1a_200 / e1_900 files when maxf == 800. Outputs go to results/experiments/k5/;
...
function main(args::Vector{String})
    ...
    eps = 0.3
    outdir = joinpath(REPO, "results", "experiments", "k5")
    do_range = true
    limit = typemax(Int)
    i = 1
    while i <= length(args)
