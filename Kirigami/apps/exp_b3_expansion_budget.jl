# exp_b3_expansion_budget -- B3 of ideas/round2_theorist_b.md: "tr K is the expansion budget".
#
...
#              machinery is exp_k7_periodic_jacobian.jl's, included below verbatim, and the CSV is diffed
#              against
#              results/experiments/k7/k7_c3_all_v2.csv) record tr K of the realised design,
...
#
#   julia --project=Kirigami Kirigami/apps/exp_b3_expansion_budget.jl [--stage run|b1|pdiag|summary]
#         [--out DIR] [--shard S] [--nshard M] [--limit K]
#
# Population: exp_k7_periodic_jacobian.jl's periodic patterns (21 tilings + 12 torus Voronoi), rebuilt from
# the bit-exact generators. Outputs go to results/experiments/b3/.
# `--limit K` stops after K patterns (or K graphs of stage b1).
include(joinpath(@__DIR__, "exp_k7_periodic_jacobian.jl"))   # population, PState, achievable set, certificate, 0+ objective
...
# The C3 free-subspace pattern search, verbatim from exp_k7_periodic_jacobian's c3 stage, factored
...
function main(args::Vector{String})
    stage = "run"
    outdir = joinpath(REPO, "results", "experiments", "b3")
    shard = 0; nshard = 1
    limit = typemax(Int)
    i = 1
    while i <= length(args)
