# apps/exp_common.jl -- the graph populations shared by every experiment and
# the app-side helpers around them.
#
# The population DEFINITIONS (`Graph`, `checkerboard`, `make_graph`, `RefCase`,
# `reference_cases`) live in the package (src/core/populations.jl) because the tests need
...
The experiment population `name` as `K.Graph`s (mesh with sigma_mc, kind, id, ok). By default the
...
# ---- shape-space cache -----------------------------------------------------------------
# The dense SVD of [L;B] costs seconds at N ~ 1400 and four experiment need the same
