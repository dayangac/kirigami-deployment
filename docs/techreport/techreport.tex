...
suites re-run for this report at 187 test sets / 176{,}692 assertions (whole suite; 23 assertions are \texttt{@test\_broken} locks on the C++ optimiser path) and 38 / 150{,}191 (\file{derivation\_tests.jl}), no failures.
\end{abstract}
...
julia --project=Kirigami -e 'using Pkg; Pkg.test()'          # 187 test sets, 176,692 assertions, 23 broken, 0 failures; includes derivation_tests.jl (38 test sets, 150,191 assertions)
\end{verbatim}
...
julia --project=Kirigami -e 'using Pkg; Pkg.test()'   # 187 test sets, 176,692 assertions, 23 broken, 0 failures; incl. derivation_tests.jl 38 test sets, 150,191 assertions
\end{verbatim}
