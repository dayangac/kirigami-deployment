...
soundness violations of the certificate. All code is C++20 with CMake; the two test
...
The reimplementation is C++20 + CMake, header-only dependencies (Eigen, nlohmann/json,
...
\textbf{[F, \file{core/kinematics.cpp}] The code's sign convention.} Face $f$ is transformed
...
\textbf{[N] Path-independence.} \file{derivations/scratch/check\_t1\_t2.cpp}, check C1: build
...
\begin{algorithm}[t]
\caption{Assembling the deploy basis $(C,S)$ --- \file{method/deploy\_basis.cpp}}
\label{alg:basis}
\begin{algorithmic}[1]
...
\textbf{A naming clash, recorded.} The header of \file{deploy\_basis.hpp} writes
...
independent BFS, against \file{kinematics.cpp::deploy()} at
...
\begin{algorithm}[t]
\caption{Exact $\Tmax$ (T4.2$''$) --- \file{method/contact.cpp::exact\_theta\_max\_overlap}}
\label{alg:tmax}
\begin{algorithmic}[1]
...
\begin{algorithm}[t]
\caption{Broad phase --- \file{method/contact.cpp::candidate\_pairs}}
\label{alg:broad}
\begin{algorithmic}[1]
...
This is exactly what \file{contact.cpp} implements --- before round 6 the derivation demanded
...
\textbf{[N] The measured counterexample} (\file{derivations/scratch/check\_t5\_adm.cpp},
...
\file{contact.cpp}; a doctest fails without it and passes with it.
...
bisection referee (\file{results/final/e1/E1.md}, driver \file{code/apps/kill\_e1.cpp}, which
...
\file{code/src/method/contact.cpp:339} --- keep a root only if it is smaller than the
...
\file{zero\_plus\_q} and \file{zero\_plus\_corner\_margin} at the $\sigma$ ray, in a doctest,
on three tilings. The flex basis is $\ker A$ (dense SVD) plus the integrated translations plus
one free translation pair per component of $\Gamma$; a doctest checks
...
A doctest pins this. \emph{Any claim of the form ``authored tilings pass 8/8'' is really
...
surrogate. Two doctests check exactly those two claims
(\file{code/tests/test\_range\_embed.cpp}), and two more finite-difference the analytic
...
objective of \file{range\_opt.hpp}, whose gradients are the implicit-function-theorem
...
\begin{algorithm}[t]
\caption{\texttt{design\_range\_max} --- the deliverable (K9c). \file{method/range\_embed.cpp}, \file{method/design.cpp}}
\label{alg:k9c}
\begin{algorithmic}[1]
...
iterates an unordered container or touches global state, and Eigen is built without OpenMP so
...
Eigen version or assembly order only if $\Phi$ is \textbf{carried along, not recomputed}.
(Verified: cached and freshly recomputed bases agree bit-for-bit on this toolchain.)
\item \file{apps/kill\_k9c.cpp} is the one place that was genuinely non-deterministic: it
...
\file{tests/test\_design.cpp} locks four rows to $10^{-9}$: \file{delaunay\_130}
...
\file{method/periodic\_jacobian.cpp} builds the genuine quotient: one face per translation
...
Everything is C++17/20 with CMake. Python appears only for matplotlib plotting of CSV/JSON
...
area & path \\
\midrule
core pipeline (2026 reimplementation) & \file{code/src/core/}: \file{mesh} \file{cut} \file{holes} \file{orientation} \file{tutte\_auxetic} \file{kinematics} \file{collision} \file{optimize} \file{rank\_checks} \file{generators} \\
...
the web explorer and compared byte for byte:
...
\file{method/design.hpp} is the single entry point, so that ``certified'' means one thing
...
\file{expansive\_cone.hpp}; the C-IRIS line supplies the general pattern of
...
./code/build/kill_k2b  --out results/kill/k2b --maxf 160   # ~26 min (needs native)
...
./code/build/kill_jitter --shard I --nshards 12 --out results/kill/jitter  # ~2 s
./code/build/kill_k7   --stage main --shard I --nshard 12 --out results/kill/k7
./code/build/kill_b3   --out results/kill/b3
./code/build/kill_b4   --n 200 --nshards 12 --shard I \
   --out results/kill/b4/shards
./code/build/kill_k8a  --n 100 --x0 --dual-iters 100000 --shard I --nshard 8 \
   --out results/kill/k8a
./code/build/kill_k9   --out results/kill/k9
./code/build/kill_k9b  --out results/kill/k9b
./code/build/kill_k9c  --out results/kill/k9c     # 12 shards; --aggregate to merge
./code/build/kill_native200 --n 200 --maxf 800 --timeout 600 \
