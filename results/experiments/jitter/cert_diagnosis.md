...
`results/experiments/jitter/pre_f32fix/`, the same effect with the metric restated as
...
**One caveat for the derivation check.** Sub-lemma T5.2b'' Case A (round 5) handles the
...
## 6. Re-measurement of A3 (`Kirigami/apps/exp_a3_jitter.jl`, 12 shards, ~2 s)
...
New `summary.txt` and `ladder.csv` are regenerated in `results/experiments/jitter/`; the pre-fix
CSVs and summary are preserved under `results/experiments/jitter/pre_f32fix/`.

## 7. Impact on earlier facts

**K2a — the numbers change, the claims survive and get stronger.** Re-ran
`julia --project=Kirigami Kirigami/apps/exp_k2a_thetamax_bisection.jl` (6.5 s, 187 configurations):

| K2a line | before (EXPERIMENTS.md L408–418) | after |
|---|---|---|
...
**K5** (`julia --project=Kirigami Kirigami/apps/exp_k5_orientation.jl`, full 200 graphs, both sigma rules, `eps = 0.3`) — identical to the
recorded numbers:
...
**K6** (`Kirigami/apps/exp_k6_zero_plus_repair.jl --n 20 --no-native`, a smoke re-run under the same code): every clause
...
`julia --project=Kirigami Kirigami/apps/exp_k6_zero_plus_repair.jl --n 20 --no-native` (40 rows = 20 graphs x 2 sigma):
