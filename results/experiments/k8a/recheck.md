...
**Short answer: the solver was wrong, but not in the direction that invalidates the certificates.**
...
## 3. Step 1 — the known-feasible test (`results/experiments/k8a/recheck_k9.csv`)

`Kirigami/apps/exp_k8a_expansive_cone_recheck.jl` regenerates K9's variant-(b) embeddings bit-identically
...
K1a population, 100 000 dual steps (`results/experiments/k8a/recheck100k/`):
...
`Kirigami/apps/exp_k8a_expansive_cone.jl`, 100 graphs, both embeddings, 8 shards, corrected solver:
...
2. K9's `expansive-cone LP: run 30, feasible 0` line in `results/experiments/k9/summary.txt`
...
for i in $(seq 0 11); do julia --project=Kirigami Kirigami/apps/exp_k8a_expansive_cone_recheck.jl --out results/experiments/k8a \
    --shard $i --nshards 12 & done; wait

# steps 2-3: the K1a population with the corrected solver
for i in $(seq 0 7); do julia --project=Kirigami Kirigami/apps/exp_k8a_expansive_cone.jl --n 100 --x0 --dual-iters 100000 \
    --out results/experiments/k8a/recheck100k --cache results/experiments/cache \
    --shard $i --nshard 8 & done; wait
```

Artifacts: `results/experiments/k8a/recheck_k9.csv`, `recheck_k9_summary.txt`,
`results/experiments/k8a/recheck/` (20 000 steps, includes the reference and deployable controls),
`results/experiments/k8a/recheck100k/` (100 000 steps, K1a only). The original `k8a.csv` and
