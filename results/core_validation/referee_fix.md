...
(the per-graph `sigma/*.json` dumps of the two K5 passes are not kept; `k5.csv` records
every orientation-derived number, and `exp_k5_orientation.jl` regenerates them).
