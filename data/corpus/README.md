...
`Kirigami.make_graph(id, min_faces, max_faces, n_cap)` (`Kirigami/src/core/kill_common.jl`)
...
| `deployable_population_2_0.2.json` | 73 configs (35 bases + 38 samples) | `deployable_population(2, 0.2)` | as above | `kill_k8a` (its 73 "deployable_*" rows) | Bases identical to `deployable_population.json` (35/35). Samples are NOT a prefix of the (8, 0.2) samples beyond the first base: `rng0` is one shared stream and each base consumes a number of normal draws that depends on `samples_per_base`, so every (samples, radius) pair is its own population and is stored separately. 0.7 MB. |
| `deployable_population_20_0.2.json` | 415 configs (35 bases + 380 samples) | `deployable_population(20, 0.2)` | as above | `kill_e1` part (A), tag `a1` | Bases identical to the (8, 0.2) file (35/35). 3.9 MB. |
| `deployable_population_20_0.35.json` | 415 configs (35 bases + 380 samples) | `deployable_population(20, 0.35)` | as above | `kill_e1` part (A), tag `a2` | Bases identical to the (8, 0.2) file (35/35). 3.9 MB. |
...
* `kill_e1.jl`'s `sigma_def` (`defect_search_at`, seed `5000000 + id`, and its authored-
...
* `kill_k5.jl`'s own `defect_search` (the run that produced `results/kill/k5/sigma`)
